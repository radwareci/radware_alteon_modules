    ...
    suboptions:
      index:
        description:
          ...
          - Alteon uses the name and the application as the identifier for all objects created for this service (group,
            SSL policy, FastView policy).
        required: true
        default: null
        type: str
      ip_ver:
      ...
      ip_address:
        description:
          - Specifies the IP version and address of the virtual server. The virtual server created within Alteon
            responds to ARPs and pings from network ports as if it was a normal server.
          - Client requests directed to the virtual server IP address are balanced among the real servers available to
            it through real server group assignments.
        required: false
        default: null
        type: str
      ip6_address:
      ...
      state:
        description:
          - Specifies whether to enable the virtual server. This option activates the virtual server so that it can
      ...
      domain_name:
        description:
          ...
          - DNS resolution for global load balancing. Additional domains can be defined on the same virtual server by
            attaching multiple DNS (GSLB) rules, each with a different domain.
          - HTTP/S health check, if the health check Host parameter is set to Inherit.
          - ""
          - The domain name typically includes the name of the company or organization, and the Internet group code
            (.com, .edu, .gov, .org, and so on). For example, 'foocorp.com'.
      ...
      availability:
        description:
          ...
          - Rules that use Availability as the primary metric handle failures by selecting the server with the next
            highest score compared to that of the server that failed, and begin forwarding requests to that server.
            If the server that failed becomes operational again, that server regains precedence and requests are
            routed to it once more.
        required: false
        default: 1
        type: int
      virtual_server_name:
      ...
      src_network_class_id:
        description:
          - Specifies the network class that defines the clients to which this virtual server provides service.
            Defining a source network per virtual server allows you to provide differentiated services for the same
            application to different clients or departments.
        required: false
        default: null
        type: str
      availability_persist:
        description:
          - Specifies whether a server that failed and became operational again, can (Disable) or cannot (Enable)
            regain precedence from the recovery server.
          - Ensuring that the former primary server does not regain precedence is achieved by assigning the highest
            possible availability value (48) to the server that takes over after a failure. If this new primary server
            fails, its original availability value is restored and the next server in the list gains the higher
            precedence.
        required: false
        default: disabled
        choices:
      ...
      wan_link_id:
        description:
          - Specifies the WAN Link via which this virtual server can be accessed, when the application can be accessed
            via multiple WAN links (multihoming).
        required: false
        default: null
        type: str
      return_to_src_mac:
        description:
          ...
          - C(enabled)-Alteon returns the response traffic to the MAC address from which the request arrived, bypassing
            all routing configuration in the device.
